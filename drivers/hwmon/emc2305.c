...
static ssize_t pwm_store(struct device *dev, struct device_attribute *attr,
			 const char *buf, size_t count)
{
	return set_pwm(dev->parent,attr,buf,count);
}

static ssize_t pwm_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	return show_pwm(dev->parent,attr,buf);
}

static ssize_t rpm_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	return show_fan_input(dev->parent,attr,buf);
}
...
emc2305_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_BYTE_DATA))
		return -EIO;
	data = devm_kmalloc(&client->dev,sizeof(struct emc2305_data),GFP_KERNEL | __GFP_ZERO);
	//data = kzalloc(sizeof(struct emc2305_data), GFP_KERNEL);
	if (!data)
		return -ENOMEM;

	data->groups[0] = &pwm_dev_group;
	data->groups[1] = &fan_dev_group;
	data->groups[2] = NULL;
...
exit_remove:
	for (i = 0; i < ARRAY_SIZE(emc2305_attr_common); ++i)
		device_remove_file(&client->dev,
				   &emc2305_attr_common[i].dev_attr);
exit_free:
	//kfree(data); don't free the data since we're using devm_kmalloc now
	return status;
}
